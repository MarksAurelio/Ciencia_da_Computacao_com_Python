...
4 / 5= 0.8
5 / 4 =0.8start fraction, 4, divided by, 5, end fraction, equals, 0, point, 8
...
3 / 5 = 0.6
5 / 3 =0.6
start fraction, 3, divided by, 5, end fraction, equals, 0, point, 6
...
​	
 ∣∣
​	
 S, start subscript, a, b, end subscript, equals, start fraction, sum, start subscript, i, equals, 1, end subscript, start superscript, 6, end superscript, vertical bar, vertical bar, f, start subscript, i, comma, a, end subscript, minus, f, start subscript, i, comma, b, end subscript, vertical bar, vertical bar, divided by, 6, end fraction
...
ab
​	
 S, start subscript, a, b, end subscript é o grau de similaridade entre os textos 
a
aa e 
b
bb;
...
i,a
​	
 f, start subscript, i, comma, a, end subscript é o valor de cada traço linguístico 
i
ii no texto 
a
aa; e
...
i,b
​	
 f, start subscript, i, comma, b, end subscript é o valor de cada traço linguístico 
i
ii no texto 
b
bb.
No nosso caso, o texto 
b
bb não é conhecido, mas temos a assinatura correspondente: a assinatura de um aluno infectado com COH-PIAH. Ou seja, sabemos o valor de 
f
i
,
b
...
i,b
​	
 f, start subscript, i, comma, b, end subscript que é dado como valor de entrada do programa. 

Caso você não esteja acostumado com a notação matemática, podemos destrinchar essa fórmula da seguinte maneira: 

Para cada traço linguístico 
i
ii (tamanho médio da palavra, relação type-token etc.) se quer a diferença entre o valor obtido em cada texto dado (
a
aa) e o valor típico do texto de uma pessoa infectada (
b
bb): 
f
i
...
i,b
​	
 f, start subscript, i, comma, a, end subscript, minus, f, start subscript, i, comma, b, end subscript
...
∣
∣
∣∣…∣∣vertical bar, vertical bar, dots, vertical bar, vertical bar), lembre-se da função abs do python.
...
6
​	
 sum, start subscript, i, equals, 1, end subscript, start superscript, 6, end superscript)
...
x
​	
 start fraction, x, divided by, 6, end fraction)

Perceba que quanto mais similares 
a
aa e 
b
bb forem, menor 
...
# --- Funções Auxiliares para cálculo de Assinatura ---

def n_palavras(lista_palavras):
    '''Esta funcao recebe uma lista de palavras e devolve o número de palavras'''
    return len(lista_palavras)

def soma_caracteres_palavras(lista_palavras):
    '''Esta funcao recebe uma lista de palavras e devolve a soma dos caracteres de todas as palavras'''
    soma = 0
    for palavra in lista_palavras:
        soma += len(palavra)
    return soma

def conta_palavras_unicas(lista_palavras):
    '''Esta funcao recebe uma lista de palavras e devolve o número de palavras únicas'''
    conjunto_palavras = set(lista_palavras)
    return len(conjunto_palavras)

def conta_hapax_legomana(lista_palavras):
    '''Esta funcao recebe uma lista de palavras e devolve o número de Hapax Legomana (palavras que aparecem uma única vez)'''
    frequencia = {}
    for palavra in lista_palavras:
...
# --- Funções Principais a serem implementadas ---

def calcula_assinatura(texto):
    '''IMPLEMENTAR. Essa funcao recebe um texto e deve devolver a assinatura do texto.
    A assinatura do texto é uma lista [wal, ttr, hlr, sal, sac, pal]'''
    
    # 1. Separar o texto em sentenças
    ...
    # 2. Iterar sobre sentenças para separar em frases e palavras
    for sentenca in sentencas:
        frases_na_sentenca = separa_frases(sentenca)
        lista_frases.extend(frases_na_sentenca)
        
        for frase in frases_na_sentenca:
            palavras_na_frase = separa_palavras(frase)
            lista_palavras.extend(palavras_na_frase)

    num_frases = len(lista_frases)
    num_palavras = n_palavras(lista_palavras)
    
    # 3. Traços Linguísticos de Palavra
    
    # Tamanho médio de palavra (wal)
    soma_carac_palavras = soma_caracteres_palavras(lista_palavras)
    wal = soma_carac_palavras / num_palavras
    
    # Relação Type-Token (ttr)
    num_palavras_diferentes = conta_palavras_unicas(lista_palavras)
    ttr = num_palavras_diferentes / num_palavras
    
    # Razão Hapax Legomana (hlr)
    num_hapax_legomana = conta_hapax_legomana(lista_palavras)
    hlr = num_hapax_legomana / num_palavras
    
    # 4. Traços Linguísticos de Sentença
    
    # Tamanho médio de sentença (sal)
    soma_carac_sentencas = 0
    for sentenca in sentencas:
        soma_carac_sentencas += len(sentenca) # Não contabiliza separadores (já removidos)
    sal = soma_carac_sentencas / num_sentencas
    
    # Complexidade de sentença (sac)
    sac = num_frases / num_sentencas
    
    # 5. Traço Linguístico de Frase
    
    # Tamanho médio de frase (pal)
    soma_carac_frases = 0
    for frase in lista_frases:
        soma_carac_frases += len(frase) # Não contabiliza separadores (já removidos)
    pal = soma_carac_frases / num_frases
    
    return [wal, ttr, hlr, sal, sac, pal]

def compara_assinatura(as_a, as_b):
    '''IMPLEMENTAR. Essa funcao recebe duas assinaturas de texto e deve devolver o grau de similaridade (um numero real).'''
...
def avalia_textos(textos, ass_cp):
    '''IMPLEMENTAR. Essa funcao recebe uma lista de textos e uma assinatura base e deve devolver o numero do texto com maior probabilidade de ter sido infectado por COH-PIAH.'''
    ...
    # O texto mais similar (maior probabilidade de COH-PIAH) é aquele com a MENOR similaridade (menor S_ab).
    
    menor_similaridade = similaridades[0]
    indice_menor = 0
    ...
    # Retorna o índice do texto mais o 1 (já que os textos são contados a partir de 1)
...
def main():
    ...
    # 2. Leitura dos Textos a serem avaliados
    textos_para_avaliar = le_textos()
    
    # 3. Avaliação e determinação do texto infectado
    if textos_para_avaliar:
        texto_infectado = avalia_textos(textos_para_avaliar, assinatura_coh_piah)
        print(f"\nO autor do texto {texto_infectado} está infectado com COH-PIAH")
    else:
        print("\nNenhum texto fornecido para avaliação.")
